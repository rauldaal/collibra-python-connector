...
class Relations(BaseAPI):
    ...
    def find_relations(
        ...
        target_id: str = None,
        type_public_id: str = None,
        source_target_logical_operator: str = "AND",
        count_limit: int = -1,
        cursor: str = None,
        limit: int = 0,
        offset: int = 0
        ...
        Args:
            ...
            relation_type_id: The ID of the type of relations to search for.
        ...
        # Pagination params: cannot use both cursor and offset
        if cursor is not None and offset not in (None, 0):
        ...
        # Validate and add relation_type_id
        if relation_type_id is not None:
            if not isinstance(relation_type_id, str):
                raise ValueError("relation_type_id must be a string")
            try:
        ...
        # typePublicId
        if type_public_id is not None:
            if not isinstance(type_public_id, str):
                raise ValueError("type_public_id must be a string")
            params["typePublicId"] = type_public_id

        # sourceTargetLogicalOperator
        if source_target_logical_operator is not None:
            if not isinstance(source_target_logical_operator, str):
                raise ValueError("source_target_logical_operator must be a string")
            op = source_target_logical_operator.upper()
            if op not in ("AND", "OR"):
                raise ValueError("source_target_logical_operator must be 'AND' or 'OR'")
            params["sourceTargetLogicalOperator"] = op
