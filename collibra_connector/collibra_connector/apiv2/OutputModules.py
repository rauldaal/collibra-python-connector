...
class OutputModules(BaseAPI):
    ...
    # Headers for sync data-streaming endpoints (no Accept to allow content negotiation)
    _DATA_HEADERS: Dict[str, str] = {"Content-Type": "application/json"}
    # ------------------------------------------------------------------
    # Internal helpers
    # ------------------------------------------------------------------
    ...
    def export_json(
        ...
        Example::
            ...
            data = conn.output_module.export_json(
                {"ViewConfig": {"Resources": {"Community": {"Id": {"name": "id"}}}}}
            )
        """
        endpoint = f"{self.__base_api}/export/json"
        params: Optional[Dict[str, Any]] = {"validationEnabled": True} if validation_enabled else None
